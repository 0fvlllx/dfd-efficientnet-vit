...
TRAINING_DIR = os.path.join(DATA_DIR, "training_set_part")  # 数据量过大会有多线程的问题
...
def read_frames(video_path, train_dataset, validation_dataset):
    ...
    if TRAINING_DIR in video_path:  # 获取训练集label
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='vit_base_r50_s16_224', type=str, metavar='MODEL',
    ...
    model = model_vit_hybrid.__dict__[opt.model](
        num_classes=1,
    )
    model.train()   
    ...
    #READ DATASET
    if opt.dataset != "All":
        folders = [opt.dataset]  # 只使用DFDC数据集
    else:
